"""
 Simple NETCONF get-config operation Python script to be used via command-line.
 ...
 Running script: (save as example.py)
    ...
    # Apply filter to get specific running configuration
    > python example.py -a 192.168.1.1 -u netconf -p netconf --port 830 --filter hostname_filter.xml
